...
> 「深渊连战」roguelike 活动(Rush 700099) · 15 把深渊武器 · 十五位自制/改造角色；当前增量链尾为 1.4.427。

> 📦 **本分支 `custom/characters-1.4.407` 内容已自洽**(分支名是建支时的锚点,不随链尾改;
> 当前链尾见下表)
>
> 服务端数据与客户端资源都在本仓,clone 即得,不需要另外装 overlay:
>
> - **服务端侧**:自制/改造角色累计 15 名(本轮新增 119997 夏可缇、149996 希尔媞)、
>   深渊限定池 990001(13 位 UP 连续置顶,两位新角色各 40356 权重、其余 11 位 10620;
>   149997 墨斯伊克本轮改为可兑换)、CN 客户端券抽 execType 3/4
> - **客户端侧**:`assets/asset-patch/active/` 增量链已从 1.4.407 补到 **1.4.427**
>   (一条边 11 个包,472 个成员;逐成员比对旧链新增 442 / 更新 30 / **删除 0**)
>
...
> | 装法 | clone 即得,服务端自动下发 | 解压到 `CDN_DIR/patches/1.4.427/`,启动期 Content Sync 激活 |
> | 链尾 | 1.4.427 | 1.4.427 |
>
> 本仓服务端**没有** Content Sync 那条装载路径,overlay 在这里装不了;反过来 dev 架构
> 也不读 `asset-patch/`。按你的服务端选一条,不要两条都装。
>
> ⚠️ **体积是单向棘轮**:`asset-patch/active/` 现在 312 MiB / 104 包,`.git` 约 800 MiB。
> zip 已压缩,git 再压收益为零(实测压缩率 100%),所以每次回灌都是永久占用。
> 本轮 20 个版本 = +36 MiB(上一轮 84 个版本 = +65 MiB)。
> **触发阈值仍是 900 MiB:再来一轮上一轮那种规模就会撞线,必须先换方案**
...
| 你是谁 | 需要什么 |
|---|---|
| **想进别人的服玩** | 不用下载本仓库。只向服主要他**按本服地址重签的完整五合一 APK**和账号说明；不要混用旧 APK。角色/模式资源由所连接服务器下发到 1.4.427 |
