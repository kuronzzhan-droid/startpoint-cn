...
export function getActiveMissionEventReleasePhase(
    ...
    for (let phase = 1; phase < maxPhase; phase++) {
        ...
        if (phaseMissions.length === 0 || !phaseMissions.every(mission => isMissionCurrentStageComplete(
            mission.missionId,
...
export function settleActiveMissionProgress(
    ...
    if (!Number.isFinite(currentProgress) || currentProgress < 0) {
        throw new TypeError("Active Mission current progress must be a finite non-negative number.")
    }
    const settledProgress = Math.max(currentProgress, authoritativeProgress)
