...
> **状态**：v1.10（2026-08-11），Claude / Codex 双侧复核补全。
...
>
> **【已修复】Claude，2026-08-11**：门禁改为三分支——两份都缺 = 合法基线（以上游 main 为基的
> worktree 本就没有）；**只剩一份 = 失败**；两份都在 = 比较内容。
> 同时补 5 个回归用例覆盖全部分支（`scripts/tests/test-hygiene.sh`，9 → 14）。
>
> **变异验证**（本项目判据：删掉判定必须变红）：
> 还原成初版的「缺失即放行」→ `AGENTS.md missing…` 用例变红；
> 单独删掉「内容分裂」那句 `note` → `content divergence` 用例变红。**测试确实打得中。**
