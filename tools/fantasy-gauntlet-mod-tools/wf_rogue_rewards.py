...
SERVER_ASSETS_DIR = Path(ROOT) / "server" / "assets"
...
TOKEN_START = "2000-01-01 12:00:00"
TOKEN_END = "2099-12-31 11:59:59"
...
# 官方 5 级武器模板的初始能力值通常是满级值的 50%；中间等级由客户端
# 线性插值。魂珠使用同一条 ability_soul 的初始值，因此自然固定为 1 级强度。
WEAPON_LEVEL1_RATIO = 0.5
...
def build_soul_leaf(
    ...
    for slot, effect in enumerate(spec.effects, start=1):
        ...
        row[45] = "5"
        row[46] = spec.group
        row[48] = str(round(effect.strength * WEAPON_LEVEL1_RATIO))
        row[49] = str(effect.strength)
        rows.append(row)
...
def build_token_leaf(template_leaf: bytes | str) -> bytes | str:
    ...
    # 官方模板只在 2025 年活动期显示；深渊连战是常驻活动，必须同步延长
    # 物品自身的展示期限，否则服务端虽已发奖，客户端仍会过滤代币图标。
    row[19] = TOKEN_START
    row[20] = TOKEN_END
    row[21] = "true"
    return _join_like([row], template_leaf)
...
def load_json(name: str):
    with open(SERVER_ASSETS_DIR / name, encoding="utf-8") as fh:
        return json.load(fh)


def save_json(name: str, data) -> None:
    with open(SERVER_ASSETS_DIR / name, "w", encoding="utf-8") as fh:
...
def main() -> int:
    ...
    try:
        from wf_rogue_validate import require_release_ready
    except ModuleNotFoundError as exc:
        print(
            "[ERR] 上游缺少 wf_rogue_validate.py，禁止走脚本内置发布；"
            "请在客户端门控验证后调用 wf_publish.py。",
            file=sys.stderr,
        )
        return 1
