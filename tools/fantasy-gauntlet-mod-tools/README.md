...
- [角色生成器方案.md](docs/角色生成器方案.md) — 角色生成器设计方案。
...
- **[MODE15_APK_INTEGRATION_DESIGN.md](docs/MODE15_APK_INTEGRATION_DESIGN.md)** — 幻想连战 APK 最终 v20 客户端移植清单：精确到类/方法、单人转多人映射、房间回程、接口复用及禁用的中间试验；配套可机读清单 `examples/mode15_client_routing.v20.json` 和检查脚本 `wf_mode15_apk_verify.py`。
